...
class GMM:
    ...
    def fit_EM(self, X, init_γ):
        ...
        while len(log_likelihoods) < max_iters:
            ...
            for k in range(self.k):
                ...
                ## means
                mu[k] = 1. / N_ks[k] * np.sum(γ[:, k] * X.T, axis = 1).T
                x_mu = np.matrix(X - mu[k])

                ## covariances
                Σ[k] = np.array(1 / N_ks[k] * np.dot(np.multiply(x_mu.T,  γ[:, k]), x_mu))
        ...
        from collections import namedtuple
        self.params = namedtuple('params', ['mu', 'Σ', 'w', 'log_likelihoods', 'num_iters'])
        self.params.mu = mu
        self.params.Σ = Σ
        self.params.w = w
    ...
    def predict(self, x):
        ...
        probs = np.array([w * p(mu, s) for mu, s, w in \
            zip(self.params.mu, self.params.Σ, self.params.w)])
...
#    np.savetxt('sample.csv', X, fmt = "%.4f",  delimiter = ",")
    ####
    gmm = GMM(3, 0.000001)
    params = gmm.fit_EM(X, max_iters= 10)
...
if __name__ == "__main__":
    ...
    X = np.array(img.reshape(img.shape[0]*img.shape[1],img.shape[2]),dtype=np.float64)
    ...
    print (gmm.predict(np.array([1, 2, 4])))